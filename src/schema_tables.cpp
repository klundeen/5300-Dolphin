...
Handle Tables::insert(const ValueDict *row) {
    // Try SELECT * FROM _tables WHERE table_name = row["table_name"] and it
...
// NOTE: once the row is deleted, any reference to the table (from get_table()
// below) is gone! So drop the table first.
...
void Tables::get_columns(Identifier table_name, ColumnNames &column_names,
    ...
    for (auto const &handle : *handles) {
        ValueDict *row = Tables::columns_table->project(
            handle); // get the row's values: {'column_name': <name>,
                     // 'data_type': <type>}
...
DbRelation &Tables::get_table(Identifier table_name) {
    // if they are asking about a table we've once constructed, then just return
    // that one
    if (Tables::table_cache.find(table_name) != Tables::table_cache.end())
...
Handle Columns::insert(const ValueDict *row) {
    ...
    if (!is_acceptable_identifier(row->at("table_name").s))
        throw DbRelationError("unacceptable table name '" +
                              row->at("table_name").s + "'");
    if (!is_acceptable_identifier(row->at("column_name").s))
        throw DbRelationError("unacceptable column name '" +
                              row->at("column_name").s + "'");
    if (!is_acceptable_data_type(row->at("data_type").s))
        throw DbRelationError("unacceptable data type '" +
                              row->at("data_type").s + "'");

    // Try SELECT * FROM _columns WHERE table_name = row["table_name"] AND
    // column_name = column_name["column_name"] and it should return nothing
    ValueDict where;
    ...
    delete handles;
    if (!unique)
        throw DbRelationError("duplicate column " + row->at("table_name").s +
                              "." + row->at("column_name").s);
...
Handle Indices::insert(const ValueDict *row) {
    ...
    if (!is_acceptable_identifier(row->at("index_name").s))
        throw DbRelationError("unacceptable index name '" +
                              row->at("index_name").s + "'");

    // Try SELECT * FROM _indices WHERE table_name = row["table_name"] AND
    ...
    if (row->at("seq_in_index").n > 1)
        where["column_name"] = row->at(
            "column_name"); // check for duplicate columns on the same index
    Handles *handles = select(&where);
    bool unique = handles->empty();
    delete handles;
    if (!unique)
        throw DbRelationError("duplicate index " + row->at("table_name").s +
                              " " + row->at("index_name").s);
    return HeapTable::insert(row);
}

// Remove a row, but first remove from index cache if there
// NOTE: once the row is deleted, any reference to the index (from get_index()
// below) is gone! So drop the index
...
void Indices::get_columns(Identifier table_name, Identifier index_name,
                          ColumnNames &column_names, bool &is_hash,
                          bool &is_unique) {
    // SELECT * FROM _indices WHERE table_name = <table_name> AND index_name =
    // <index_name>
    ValueDict where;
    where["table_name"] = table_name;
...
class DummyIndex : public DbIndex {
  public:
    DummyIndex(DbRelation &rel, Identifier idx, ColumnNames key, bool unq)
        : DbIndex(rel, idx, key, unq) {}
...
DbIndex &Indices::get_index(Identifier table_name, Identifier index_name) {
    // if they are asking about an index we've once constructed, then just
    // return that one
    std::pair<Identifier, Identifier> cache_key(table_name, index_name);
    if (Indices::index_cache.find(cache_key) != Indices::index_cache.end())
...
IndexNames Indices::get_index_names(Identifier table_name) {
    ...
    where["seq_in_index"] =
        Value(1); // only get the row for the first column if composite index
    Handles *handles = select(&where);
    for (auto const &handle : *handles) {
        ValueDict *row = project(handle);
